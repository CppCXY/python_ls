use crate::{
    kind::PyTokenKind,
    lexer::{LuaDocLexer, LuaDocLexerState, PyTokenData},
...
impl<'b> LuaDocParser<'_, 'b> {
    pub fn parse(lua_parser: &mut LuaParser<'_>, tokens: &[PyTokenData]) {
    ...
    fn calc_next_current_token(&mut self) {
        ...
        if self.current_token == PyTokenKind::TkEof {
            return;
        }
        ...
        match self.lexer.state {
            ...
            LuaDocLexerState::Init => {
                while matches!(
                    self.current_token,
                    PyTokenKind::TkEndOfLine | PyTokenKind::TkWhitespace
                ) {
                    self.eat_current_and_lex_next();
                }
            }
    ...
    fn lex_token(&mut self) -> PyTokenData {
        #[allow(unused_assignments)]
        let mut kind = PyTokenKind::TkEof;
        loop {
            if self.lexer.is_invalid() {
                let next_origin_index =
                    if self.origin_token_index == 0 && self.current_token == PyTokenKind::None {
                        0
                    } else {
                        self.origin_token_index + 1
                    };
                if next_origin_index >= self.tokens.len() {
                    return PyTokenData::new(
                        PyTokenKind::TkEof,
                ...
                if next_origin_token.kind == PyTokenKind::TkEndOfLine
                    || next_origin_token.kind == PyTokenKind::TkWhitespace
                    || next_origin_token.kind == PyTokenKind::TkShebang
                {
        ...
        PyTokenData::new(kind, self.lexer.current_token_range())
    }
        ...
        match state {
            LuaDocLexerState::Description => {
                if !matches!(
                    ...
                    PyTokenKind::TkWhitespace
                        ...
                        | PyTokenKind::TkLongCommentStart
                        | PyTokenKind::TkDocStart
                        | PyTokenKind::TkDocLongStart
                        | PyTokenKind::TkLongCommentEnd
                ) {
                    self.re_calc_detail();
                }
            }
            LuaDocLexerState::Trivia => {
                if !matches!(
                    ...
                    self.current_token = PyTokenKind::TkDocTrivia;
                }
            }
    ...
    fn re_calc_detail(&mut self) {
        self.current_token = PyTokenKind::TkDocDetail;
        if self.lexer.is_invalid() {
            return;
        }
        self.current_token = PyTokenKind::None;
...
fn is_invalid_kind(kind: PyTokenKind) -> bool {
    matches!(kind, PyTokenKind::None | PyTokenKind::TkEof)
}
