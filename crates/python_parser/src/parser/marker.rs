...
pub(crate) trait MarkerEventContainer {
    ...
    fn mark(&mut self, kind: PySyntaxKind) -> Marker {
...
impl Marker {
    ...
    pub fn set_kind<P: MarkerEventContainer>(&mut self, p: &mut P, kind: PySyntaxKind) {
    ...
    pub fn complete<P: MarkerEventContainer>(self, p: &mut P) -> CompleteMarker {
        ...
        if finish == self.position + 1 {
            match &mut p.get_events()[self.position] {
                MarkEvent::NodeStart { kind, .. } => *kind = PySyntaxKind::None,
                _ => unreachable!(),
            }
