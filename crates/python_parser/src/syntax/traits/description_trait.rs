use crate::{LuaAstNode, LuaDocDescription, LuaKind, PySyntaxKind, LuaSyntaxNode, PyTokenKind};
...
fn check_is_inline_description(node: &LuaSyntaxNode) -> Option<bool> {
    ...
    for _ in 0..=3 {
        ...
        if let Some(sibling) = &prev_sibling {
            match sibling.kind() {
                LuaKind::Token(PyTokenKind::TkWhitespace | PyTokenKind::TkDocContinue) => {}
                LuaKind::Token(PyTokenKind::TkEndOfLine | PyTokenKind::TkNormalStart) => {
                    return Some(false);
                }
                _ => {
                    return Some(true);
