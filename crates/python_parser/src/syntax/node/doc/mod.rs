...
fn is_additive_doc_tag(kind: PySyntaxKind) -> bool {
    matches!(
        kind,
        PySyntaxKind::DocTagVisibility
...
impl LuaComment {
    ...
    pub fn get_description(&self) -> Option<LuaDocDescription> {
        for child in self.syntax.children_with_tokens() {
            match child.kind() {
                LuaKind::Syntax(PySyntaxKind::DocDescription) => {
                    return LuaDocDescription::cast(child.into_node().unwrap());
                }
                LuaKind::Token(PyTokenKind::TkDocStart) => {}
...
fn find_inline_node(comment: &LuaSyntaxNode) -> Option<LuaSyntaxNode> {
    ...
    loop {
        ...
        if let Some(sibling) = prev_sibling {
            match sibling.kind() {
                LuaKind::Token(
                    PyTokenKind::TkWhitespace | PyTokenKind::TkComma | PyTokenKind::TkSemicolon,
                ) => {}
                LuaKind::Token(PyTokenKind::TkEndOfLine)
                | LuaKind::Syntax(PySyntaxKind::Comment) => {
                    return None;
                }
                LuaKind::Token(k) if k != PyTokenKind::TkName => {
    ...
    loop {
        ...
        if let Some(sibling) = next_sibling {
            match sibling.kind() {
                ...
                LuaKind::Token(PyTokenKind::TkWhitespace) => {}
                LuaKind::Syntax(PySyntaxKind::Comment) => {
                    return None;
                }
...
impl LuaAstNode for LuaDocGenericDeclList {
    ...
    where
        Self: Sized,
    {
        kind == PySyntaxKind::DocGenericDeclareList
    }
...
impl LuaAstNode for LuaDocGenericDecl {
    ...
    where
        Self: Sized,
    {
        kind == PySyntaxKind::DocGenericParameter
    }
...
impl LuaAstNode for LuaDocObjectField {
    ...
    where
        Self: Sized,
    {
        kind == PySyntaxKind::DocObjectField
    }
    ...
    pub fn get_field_key(&self) -> Option<LuaDocObjectFieldKey> {
        for child in self.syntax.children_with_tokens() {
            match child.kind() {
                ...
                LuaKind::Token(PyTokenKind::TkColon) => {
                    return None;
                }
                _ => {}
            }
...
impl LuaAstNode for LuaDocAttribute {
    ...
    where
        Self: Sized,
    {
        kind == PySyntaxKind::DocAttribute
    }
...
impl LuaAstNode for LuaDocNamedReturnType {
    ...
    where
        Self: Sized,
    {
        kind == PySyntaxKind::DocNamedReturnType
    }
